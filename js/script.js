...
function showToast(message, type = 'warning') {
  ...
  el.innerHTML = `<div class="d-flex"><div class="toast-body fw-semibold">${message}</div><button type="button" class="btn-close btn-close-white me-2 m-auto" data-bs-dismiss="toast" aria-label="Cerrar"></button></div>`;
...
function setTrackNameHtml(el, trackName) {
  ...
  if (idx === -1) {
    el.innerHTML = `<strong>${trackName}</strong>`;
  } else {
    const title = trackName.slice(0, idx);
    const body  = trackName.slice(idx + 2).replace(/\n/g, '<br>');
    el.innerHTML = `<strong>${title}</strong><br><br>${body}`;
  }
}
...
function showFinalRanking() {
  ...
  teams.forEach((t, idx) => {
    ...
    div.innerHTML = `<div class="rank-item-content"><div class="rank-item-icon" style="background:${t.color}"></div><div class="rank-name">${idx+1}. ${t.name}</div></div><div class="rank-score">${t.score} Pts</div>`;
